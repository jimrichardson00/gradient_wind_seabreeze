...
metar_summerday_df <- ddply(.data = metar_summerday
	...
	, windwes_met = ifelse(any(winddir >= 230 & winddir <= 330 & windkno >= 10), 1, 0)
...
plot(density(na.omit(metar_summerday_df$winddir_avg)))
...
metar_summerday_sum <- ddply(.data = metar_summerday_mer
	...
	, tempera_utc = mean(na.omit(tempera_utc))
	, dewpntt_met = mean(na.omit(dewpntt_met))
	, dewpntt_utc = mean(na.omit(dewpntt_utc))
...
plot(metar_summerday_mer$windkno_upp_rnd, metar_summerday_mer$winddir_avg)
plot(metar_summerday_mer$windkno_upp_rnd, metar_summerday_mer$windkno_avg)
plot(metar_summerday_mer$windkno_upp_rnd, metar_summerday_mer$tempera_met)
plot(metar_summerday_mer$windkno_upp_rnd, metar_summerday_mer$dewpntt_met)
...
metar_westerly <- metar_westerly[(metar_westerly$windkno >= 10), ]
metar_westerly <- metar_westerly[(metar_westerly$hourloc >= 9) & (metar_westerly$hourloc <= 18), ]
metar_westerly <- metar_westerly[(metar_westerly$month_loc %in% c(8, 9, 10, 11, 12, 1, 2)), ]
head(metar_westerly)
...
metar_temperature <- metar_temperature[(metar_temperature$hourloc >= 9) & (metar_temperature$hourloc <= 18), ]
...
plot(density(na.omit(metar_temperature_mer$windkno_upp)))
...
metar_dewpoint <- metar_dewpoint[metar_dewpoint$dewpntt <= 0, ]
metar_dewpoint <- metar_dewpoint[(metar_dewpoint$hourloc >= 9) & (metar_dewpoint$hourloc <= 18), ]
